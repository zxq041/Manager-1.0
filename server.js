// server.js — API + serwowanie index.html/assetów z katalogu głównego (bez folderu public)
...
/* =========================
   1) STATYKI Z KATALOGU GŁÓWNEGO
   ========================= */
// Serwuj wszystkie pliki leżące obok server.js (index.html, sw.js, manifest.json, favicon.png, itp.)
app.use(express.static(__dirname, {
  // dzięki temu / otworzy index.html jeśli istnieje
  extensions: ['html'],
  // lekkie cache statyków (Railway i tak daje CDN/proxy)
  maxAge: '1h',
}));

/* =========================
   2) BAZA: Mongo (opcjonalnie) lub pamięć
   ========================= */
...
(async () => {
  if (process.env.MONGODB_URI) {
    try {
      ...
      if (cnt === 0) {
        await Models.User.create({ login:'Gracjan', password:'Gracjan33201', role:'admin' });
      }
...
app.post('/api/orders/:id/toggle', async (req,res)=>{
  ...
  o.done = !o.done;
  if (o.done) mem.finance.unshift({ type:'income', amount:o.amount||0, note:`Zlecenie: ${o.title}` });
  ok(res,o);
});
...
/* =========================
   4) SPA FALLBACK
   ========================= */
// Jeśli żądanie to NIE /api/* i nie ma fizycznego pliku, zwróć index.html
